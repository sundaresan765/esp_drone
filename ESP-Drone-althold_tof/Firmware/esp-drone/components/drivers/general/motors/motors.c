...
#include "config.h"
//#include "general/buzzer/include/piezo.h"
#include "log.h"
...
bool pwm_timmer_init()
{
    ...
    printf("pwm is configured\n");
    // Set configuration of timer0 for high speed channels
    if (ledc_timer_config(&ledc_timer) == ESP_OK) {
        isTimerInit = TRUE;
        return TRUE;
    }
    printf("pwm is finished\n");
    return FALSE;
}
bool spwm_timmer_init()
{
...
void motorsInit(const MotorPerifDef **motorMapSelect)
{
    ...
    if (pwm_timmer_init() != TRUE) {
        printf("it is in pwm timmer");
        return;
    }

    for (i = 0; i < NBR_OF_MOTORS; i++) {
         printf("it is in  motors pwm timmer");
...
void ismotorsInit(const MotorPerifDef **motorMapSelect)
{
    int i;

    printf("ismotorinit inside a function\n");
    motorMap = motorMapSelect;

    if (pwm_timmer_init() != true) {
        printf("pwm inside a function\n");
        return;
    }
    printf("ismotorinit inside motor a function");
    printf("number of motors %d\n", NBR_OF_MOTORS);

    for (i = 0; i < NBR_OF_MOTORS; i++) {
        printf("inside a led channel\n");
        ledc_channel_config(&motors_channel[i]);
        printf(" inside a led channel finished\n");
    }
    //spiezoInit();
    spwm_timmer_init();
...
bool smotorsTest(void)

{
     motorsSetRatio(MOTORS[0],1300);
     motorsSetRatio(MOTORS[1],1300);
     motorsSetRatio(MOTORS[2], 1300);
     motorsSetRatio(MOTORS[3],1300);
    vTaskDelay(M2T(MOTORS_TEST_ON_TIME_MS));
    ...
    //          printf("power distribution is called else\n");
              
    //         motorsSetRatio(MOTORS[i],65000);
    //         motorsSetRatio(MOTORS[i], 65000);
    //         printf("else power distribution,%d", MOTORS_TEST_RATIO);
    //         vTaskDelay(M2T(MOTORS_TEST_ON_TIME_MS));
...
bool motorsTest(void)
{
    ...
    for (i = 0; i < sizeof(MOTORS) / sizeof(*MOTORS); i++) {
        if (motorMap[i]->drvType == BRUSHED) {
#ifdef ACTIVATE_STARTUP_SOUND
             printf("power distribution is called\n");
            motorsBeep(MOTORS[i], true, testsound[i], (uint16_t)(MOTORS_TIM_BEEP_CLK_FREQ / A4) / 20);
            vTaskDelay(M2T(MOTORS_TEST_ON_TIME_MS));
